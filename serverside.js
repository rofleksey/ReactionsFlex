	...
	if(req.query.id && reactions.find(r => r.id == req.query.id)) {
		...
		if(t) {
			res.render('watch', Object.assign(defMust, {
				streamURL: JSON.stringify(t.link)
			}));
		} else if(r.state == ReactionState.DONE && r.link) {
			res.render('watch', Object.assign(defMust, {
				streamURL: JSON.stringify(r.link)
			}));
		} else {
			res.status(404).render('error404')
		}
...
filesServer.on('connection', function(ws) {
	...
	var arr = reactions.map(r => r.getJSON()).reverse();
...
class Task {
	constructor (react, rtime, etime, volume, scale, where, skiptime, isTest) {
		this.rtime = rtime
		this.etime = etime
		this.react = react
		this.volume = volume
	...
	execute(onMsg, onCommand, onSuccess, onFail) {
		try {
			...
			if(this.skiptime != '0:00') {
				var rlen = (rtime.match(/\:/g) || []).length;
				var elen = (etime.match(/\:/g) || []).length;
				var skiplen = (this.skiptime.match(/\:/g) || []).length;
				console.log(rlen, elen)
				var rm = moment.duration((rlen < 2 ? '0:' : '') + rtime)
				var em = moment.duration((elen < 2 ? '0:' : '') + etime)
				var skipm = moment.duration((skiplen < 2 ? '0:' : '') + this.skiptime)
				var result_rm = rm.add(skipm)
				var result_em = em.add(skipm)
				rtime = moment.utc(result_rm.asMilliseconds()).format("HH:mm:ss.S")
				etime = moment.utc(result_em.asMilliseconds()).format("HH:mm:ss.S")
			}
			if(!this.isTest) {
				array = ['-ss', rtime, '-i', `${__dirname}/www/${this.react.reactionPath}`, '-ss', etime, '-i', `${__dirname}/www/${this.react.episodePath}`, '-filter_complex',
...
testServer.on('connection', function(ws) {
	...
	ws.on('message', function(msg){
		...
		if(id && reactions.find(r => r.id == id)) {
			...
			ws.task = new Task(r, data.rtime, data.etime, data.volume, data.scale, data.where, data.skiptime, true)
...
processServer.on('connection', function(ws) {
	...
	ws.on('message', function(msg){
		...
		if(id && reactions.find(r => r.id == id)) {
			...
			if(!oldtask) {
				var task = new Task(r, data.rtime, data.etime, data.volume, data.scale, data.where, data.skiptime, false)
				tasks.push(task)
...
var operationsServer = new wss({noServer: true})
operationsServer.on('connection', function(ws) {
	console.log("operations::connection")
	ws.on('message', function(msg){
		var data = JSON.parse(msg)
		var r = reactions.find(r => r.id == data.id)
		if(r) {
			if(data.command == 'undo') {
				if(r.state == ReactionState.WORKING) {
					var t = tasks.find(it => it.react == r)
					r.setState(ReactionState.CREATED, null)
					if(t) {
						t.stop()
						ws.send(JSON.stringify({
							'type': 'success',
							'msg': "Reaction's metadata has been cleared"
						}))
					} else {
						ws.send(JSON.stringify({
							'type': 'error',
							'msg': "Can't find task for target reaction"
						}))
					}
				} else {
					r.setState(ReactionState.CREATED, null)
				...
				if (fs.existsSync(dir)){
					...
					ws.send(JSON.stringify({
						'type': 'success',
						'msg': "Can't find folder with reaction"
					}))
				}
			} else {
				ws.send(JSON.stringify({
...
http.on('upgrade', function upgrade(request, socket, head) {
    ...
    if (pathname === '/test') {
        ...
        operationsServer.handleUpgrade(request, socket, head, function done(ws) {
            operationsServer.emit('connection', ws, request);
        });
    } else {
        socket.destroy();
    }
